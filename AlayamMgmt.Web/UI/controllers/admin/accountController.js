'use strict';
app.controller('accountController', ['$scope', '$filter', 'sharedService', 'authService', 'ngTableParams', 'lookupService',
    function ($scope, $filter, sharedService, authService, ngTableParams, lookupService) {
            ...
            lookupService.getAccountsByOrgId($scope.authentication.orgId, null).then(function (results) {
                ...
                $scope.tableParams = new ngTableParams({
                    ...
                    getData: function ($defer, params) {
                        ...
                        var filteredData = $filter('filter')($scope.accounts, $scope.filterText);
        ...
        $scope.initialize = function (id) {
            ...
            if (id != 0) {
                var results = $filter('filter')($scope.accounts, { id: id });
        ...
        $scope.enable = function (id, status) {
            ...
            var orgOfferingTypes = $filter('filter')($scope.accounts, { id: id });
