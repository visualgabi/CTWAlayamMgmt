'use strict';
app.controller('eventTypeController', ['$scope', '$filter', 'sharedService', 'authService', 'ngTableParams', 'lookupService', 
    function ($scope, $filter, sharedService, authService, ngTableParams, lookupService ) {
                ...
                lookupService.getEventTypesByOrgId($scope.authentication.orgId, null).then(function (results) {
                    ...
                    $scope.tableParams = new ngTableParams({
                        ...
                        getData: function ($defer, params) {
                            ...
                            var filteredData = $filter('filter')($scope.eventTypes, $scope.filterText);
        ...
        $scope.initialize = function (id) {            
            ...
            if (id != 0) {
                var results = $filter('filter')($scope.eventTypes, { id: id });
        ...
        $scope.enable = function (id, status) {
            ...
            var orgOfferingTypes = $filter('filter')($scope.eventTypes, { id: id });
