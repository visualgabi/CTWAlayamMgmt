'use strict';
app.controller('expenseTypeController', ['$scope', '$filter', 'sharedService', 'authService', 'ngTableParams', 'lookupService', 
    function ($scope, $filter, sharedService, authService, ngTableParams, lookupService ) {
                ...
                lookupService.getExpenseTypesByOrgId($scope.authentication.orgId, null).then(function (results) {
                    ...
                    $scope.tableParams = new ngTableParams({
                        ...
                        getData: function ($defer, params) {
                            ...
                            var filteredData = $filter('filter')($scope.expenseTypes, $scope.filterText);
        ...
        $scope.initialize = function (id) {            
            ...
            if (id != 0) {
                var results = $filter('filter')($scope.expenseTypes, { id: id });
        ...
        $scope.enable = function (id, status) {
            ...
            var orgOfferingTypes = $filter('filter')($scope.expenseTypes, { id: id });
