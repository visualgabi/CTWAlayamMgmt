'use strict';
app.controller('depositRptController', ['$scope', 'lookupService', 'depositService', '$filter', 'sharedService', 'authService', '$window', 'PDFConveterService', 'ngAuthSettings', 'userService',
    function ($scope, lookupService, depositService, $filter, sharedService, authService, $window, PDFConveterService, ngAuthSettings, userService) {
    ...
    $scope.print = function () {
        ...
        if ($scope.depositSearch.accountId != null )
            accountName = $filter('filter')($scope.accounts, { id: $scope.depositSearch.accountId })[0].name;
                ...
                if ($scope.deposits.length > 0) {
                    ...
                    angular.forEach($scope.accounts, function (item) {
                        if ($scope.depositSearch.accountId == null || $scope.depositSearch.accountId == item.id) {
                            var filteredData = $filter('filter')($scope.deposits, { accountId: item.id });
