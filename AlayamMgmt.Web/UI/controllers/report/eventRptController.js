'use strict';
app.controller('eventRptController', ['$scope', 'lookupService', 'eventService', '$filter', 'sharedService', 'authService', '$window', 'PDFConveterService', 'ngAuthSettings',
    function ($scope, lookupService, eventService, $filter, sharedService, authService, $window, PDFConveterService, ngAuthSettings) {
    ...
    $scope.print = function () {
        ...
        if ($scope.eventSearch.splEventTypeId != "")
            eventTypeName = $filter('filter')($scope.splEventTypes, { id: $scope.eventSearch.splEventTypeId })[0].name;

        if ($scope.eventSearch.eventTypeId != null)
            subEventTypeName = $filter('filter')($scope.eventTypes, { id: $scope.eventSearch.eventTypeId })[0].name;
        ...
        if (reportForm.$valid) {
            ...
            eventService.report($scope.eventSearch).then(function (results) {
                ...
                if ($scope.events.length > 0) {
                    ...
                    $scope.splEventTotal = $filter('filter')($scope.events, { isSpecialEvent: true }).length;
                    $scope.regEventTotal = $filter('filter')($scope.events, { isSpecialEvent: false }).length;

                    var splEvents = $filter('filter')($scope.events, { isSpecialEvent: true })
                    var regEvents = $filter('filter')($scope.events, { isSpecialEvent: false })
