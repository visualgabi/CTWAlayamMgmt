'use strict';
app.controller('depositController', ['$scope', 'lookupService', 'depositService', '$filter', 'sharedService', 'authService', 'ngTableParams', 'userService',
        ...
        depositService.getByOrgId($scope.authentication.orgId, null).then(function (results) {
            ...
            $scope.tableParams = new ngTableParams({
                ...
                getData: function ($defer, params) {
                    ...
                    var filteredData = $filter('filter')($scope.deposits, $scope.filterText);
    ...
    $scope.initialize = function (id) {
        ...
        if (id != 0) {
            var results = $filter('filter')($scope.deposits, { id: id });
    ...
    $scope.enable = function (id, status) {
        ...
        var orgDeposits = $filter('filter')($scope.deposits, { id: id });
