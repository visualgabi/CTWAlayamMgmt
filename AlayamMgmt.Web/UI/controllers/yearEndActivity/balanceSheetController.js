'use strict';
app.controller('balanceSheetController', ['$scope', 'yearlyActivityService', '$filter', 'orgFiscalYearService',
    ...
    $scope.availableBalance = function (items) {
        ...
        if (items.length > 0) {
            var openingBalance = $filter('filter')(items, { orderId: 1 })[0].amount;
            var amountReceived = $filter('filter')(items, { orderId: 2 })[0].amount;
            var expenseAmount = $filter('filter')(items, { orderId: 3 })[0].amount;
    ...
    $scope.availableBalanceQuarter = function (items) {
        ...
        if (items.length > 0) {
            var amountReceived = $filter('filter')(items, { orderId: 1 })[0].amount;
            var expenseAmount = $filter('filter')(items, { orderId: 2 })[0].amount;
