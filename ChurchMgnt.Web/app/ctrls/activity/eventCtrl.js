'use strict';
app.controller('eventCtrl', ['$scope', 'lookupService', 'eventService', 'organizationService', '$filter', 'sharedService', 'authService', 'ngTableParams',
    ...
    $scope.initialize = function (id) {
        ...
        if (id != 0) {
            var results = $filter('filter')($scope.events, { id: id }, true);
    ...
    $scope.enable = function (id, status) {
        ...
        var orgEvents = $filter('filter')($scope.events, { id: id }, true);
