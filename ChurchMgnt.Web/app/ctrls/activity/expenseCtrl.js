'use strict';
app.controller('expenseCtrl', ['$scope', 'lookupService', 'expenseService', '$filter', 'sharedService', 'authService', 'ngTableParams', 'organizationService', 
    ...
    $scope.initialize = function (id) {
        ...
        if (id != 0) {
            var results = $filter('filter')($scope.expenses, { id: id }, true);
    ...
    $scope.enable = function (id, status) {
        ...
        var orgExpenses = $filter('filter')($scope.expenses, { id: id }, true);
