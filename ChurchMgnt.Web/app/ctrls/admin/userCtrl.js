'use strict';
app.controller('userCtrl', ['$scope', '$filter', 'sharedService', 'authService', 'ngTableParams', 'userService', 'roleService', 'organizationService','profileService',
    function ($scope, $filter, sharedService, authService, ngTableParams, userService, roleService, organizationService, profileService) {
    ...
    $scope.initialize = function (id) {        
        ...
        if (id != 0) {
            var results = $filter('filter')($scope.users, { id: id }, true);
   ...
   $scope.clear = function()
   {
    ...
    $scope.enable = function (id, status) {
        ...
        var orgUsers = $filter('filter')($scope.users, { id: id }, true);
    ...
    $scope.resetPassword = function (id) {
        ...
        $scope.resetPasswordData = {
            ...
            password: "",
            userEmail: orgUsers[0].userName
        }

        profileService.resetPassword($scope.resetPasswordData).then(function (results) {

            orgUsers[0].rowTimeStamp = results.data;

            $scope.alertClassForDelete = "alert-success";
            $scope.alertMsgForDelete = "User password got reseted successfully";
            $scope.showAlertForDelete = true;
