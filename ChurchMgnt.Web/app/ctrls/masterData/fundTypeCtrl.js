'use strict';
app.controller('fundTypeCtrl', ['$scope', '$filter', 'sharedService', 'authService', 'ngTableParams', 'lookupService',
    function ($scope, $filter, sharedService, authService, ngTableParams, lookupService ) {
                ...
                lookupService.getFundTypesByOrgId($scope.authentication.orgId, null).then(function (results) {
                    ...
                    $scope.tableParams = new ngTableParams({
                        ...
                        getData: function ($defer, params) {
                            ...
                            $scope.filteredData = $filter('filter')($scope.fundTypes, $scope.filterText);
        ...
        $scope.initialize = function (id) {            
            ...
            if (id != 0) {
                var results = $filter('filter')($scope.fundTypes, { id: id }, true);
        ...
        $scope.enable = function (id, status) {
            ...
            var orgFundTypes = $filter('filter')($scope.fundTypes, { id: id },true);
