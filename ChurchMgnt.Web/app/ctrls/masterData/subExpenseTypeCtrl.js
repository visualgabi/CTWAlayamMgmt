'use strict';
app.controller('subExpenseTypeCtrl', ['$scope', '$filter', 'sharedService', 'authService', 'ngTableParams', 'lookupService', 
    function ($scope, $filter, sharedService, authService, ngTableParams, lookupService) {
                ...
                lookupService.getSubExpenseTypesByOrgId($scope.authentication.orgId, null).then(function (results) {
                    ...
                    $scope.tableParams = new ngTableParams({
                        ...
                        getData: function ($defer, params) {
                            ...
                            $scope.filteredData = $filter('filter')($scope.subExpenseTypes, $scope.filterText);
        ...
        $scope.initialize = function (id) {            
            ...
            if (id != 0) {
                var results = $filter('filter')($scope.subExpenseTypes, { id: id },true);
        ...
        $scope.enable = function (id, status) {
            ...
            var orgOfferingTypes = $filter('filter')($scope.subExpenseTypes, { id: id },true);
