'use strict';
app.controller('sponsorCtrl', ['$scope', 'lookupService', 'sponsorService', '$filter', 'sharedService', 'authService', 'ngTableParams',
        ...
        sponsorService.getByOrgId($scope.authentication.orgId, null).then(function (results) {
            ...
            $scope.tableParams = new ngTableParams({
                ...
                getData: function ($defer, params) {
                    ...
                    $scope.filteredData = $filter('filter')($scope.sponsors, $scope.filterText);
    ...
    $scope.initialize = function (id) {
        ...
        if (id != 0) {
            var results = $filter('filter')($scope.sponsors, { id: id },true);
    ...
    $scope.enable = function (id, status) {
        ...
        var orgSponsors = $filter('filter')($scope.sponsors, { id: id },true);
