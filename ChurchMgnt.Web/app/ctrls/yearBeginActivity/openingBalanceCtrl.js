'use strict';
app.controller('openingBalanceCtrl', ['$scope', '$filter', 'sharedService', 'authService', 'ngTableParams', 'lookupService','openingBalanceService','orgFiscalYearService',
    function ($scope, $filter, sharedService, authService, ngTableParams, lookupService, openingBalanceService, orgFiscalYearService) {
       ...
       // $scope.accounts = [];
            ...
            openingBalanceService.getByOrgId($scope.authentication.orgId, null).then(function (results) {
                ...
                $scope.tableParams = new ngTableParams({
                    ...
                    getData: function ($defer, params) {
                        ...
                        $scope.filteredData = $filter('filter')($scope.openingBalances, $scope.filterText);
        ...
        $scope.initialize = function (id) {
            ...
            if (id != 0) {
                var results = $filter('filter')($scope.openingBalances, { id: id },true);
        ...
        $scope.enable = function (id, status) {
            ...
            var orgOfferingTypes = $filter('filter')($scope.openingBalances, { id: id },true);
