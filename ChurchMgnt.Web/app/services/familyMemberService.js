...
app.factory('familyMemberService', ['$http', '$q', 'ngAuthSettings', function ($http, $q, ngAuthSettings) {
    ...
    var _getTaxPayerByFamilyId = function (familyId, active) {
        return $http.get(serviceBase + 'api/familyMember/GetTaxPayerByFamilyId?familyId=' + familyId + '&active=' + active).then(function (results) {
            return results;
        });
    };


    var _getGroupFamilyMembersByOrgId = function (orgId) {
        return $http.get(serviceBase + 'api/familyMember/GetGroupFamilyMembersByOrgId?orgId=' + orgId).then(function (results) {
            return results;
        });
    };
    ...
    var _movefamilyMember = function (data) {

        var deferred = $q.defer();

        $http.post(serviceBase + 'api/familyMember/moveFamilyMember', JSON.stringify(data), { headers: { 'Content-Type': 'application/json' } }).then(function (successResponse) {
            deferred.resolve(successResponse);
        }, function (failerResponse) {
            deferred.reject(failerResponse);
        });
    ...
    familyMemberServiceFactory.getTaxPayerByFamilyId = _getTaxPayerByFamilyId;    
    familyMemberServiceFactory.movefamilyMember = _movefamilyMember;
    familyMemberServiceFactory.getGroupFamilyMembersByOrgId = _getGroupFamilyMembersByOrgId;
