...
{
    public class OfferingPdfHtmlHelper : IPdfHtmlHelper
    {
        public StringBuilder GenerateBody(PrintBaseModel printBaseModel)
        {
            OfferingPrintModel model = (OfferingPrintModel)printBaseModel;

            int currentPage = 1;
            int extraPage = model.Offerings.Count() % 25 == 0 ? 1 : 2;
            int totalPage = (model.Offerings.Count() / 25) + extraPage;

            StringBuilder offeringPrintHtml = new StringBuilder(Constant.OFFERING_HTML);
            ...
            StringBuilder filter = new StringBuilder("<table class='table table-bordered table-striped'><tr><th>Offering Start Date</th><td>"
                + "[startDate]</td></tr><tr><th>Offering End Date</th><td>[endDate]</td></tr><tr><th>Source Type</th><td>[sourceType]</td></tr><tr><th>Offering Type</th><td>[offeringType]</td></tr><tr><th>Fund Type</th><td>[fundType]</td></tr><tr><th>Order By</th><td>[oderBy]</td></tr></table>");
            ...
            filter.Replace("[sourceType]", model.Filter.Source);
            filter.Replace("[fundType]", model.Filter.FundType);
            filter.Replace("[offeringType]", model.Filter.FundType);
            filter.Replace("[oderBy]", model.Filter.OrderBy);

            offeringPrintHtml.Replace("[filter]", filter.ToString());

            StringBuilder summaryBySourceType = new StringBuilder("<table class='table table-bordered table-striped'><tr><th>Source type</th><th>Total in [currency]</th></tr>");
            for (int iPos = 0; iPos < model.SummaryBySourceTypes.Count; iPos++)
            {
                summaryBySourceType.AppendLine("<tr><td>" + model.SummaryBySourceTypes[iPos].Name + "</td><td>" + model.SummaryBySourceTypes[iPos].Total.ToString() + "</td></tr>");
            }
            summaryBySourceType.AppendLine("</table>");
            offeringPrintHtml.Replace("[summaryBySourceType]", summaryBySourceType.ToString());
            ...
            for (int iPos = 0; iPos < model.Offerings.Count; iPos++)
            {
                if (includeTableHeader == true)
                {
                    ...
                    expenseHmtl.AppendLine("<div style='page-break-before: always'><div class='row'><div class='col-md-12'>&nbsp;</div><div class='col-md-12'>&nbsp;</div></div><div class='row'><div class='form-group'><div class='col-md-12'><table style = 'width:100%'><tr>" +
                        "<td style = 'text-align: right'><b>Page :</b> [currentPage] of [totalPage]</td></tr></table></div></div></div> <div class='col-md-12'>&nbsp;</div><hr /><div class='col-md-12'>&nbsp;</div><table id='tblExpense' class='table table-bordered table-striped'><tr><th> Source</th><th>Member Name</th><th>Offering Type</th><th>Offering Type</th><th>Payment Type</th><th>Amount in [currency]</th><th>OfferingDate</th></tr>");
                    expenseHmtl.Replace("[currentPage]", currentPage.ToString());
                    expenseHmtl.Replace("[totalPage]", totalPage.ToString());
                }

                expenseHmtl.AppendLine("<tr><td>[source]</td><td>[memberName]</td><td>[fundType]</td><td>[offeringType]</td><td>[paymentType]</td><td>[amount]</td><td>[offeringDate]</td></tr>");
                expenseHmtl.Replace("[source]", model.Offerings[iPos].FamilyMember == null ? "Sponsor" : "Family");
                expenseHmtl.Replace("[memberName]", model.Offerings[iPos].FamilyMember != null ? model.Offerings[iPos].FamilyMember : model.Offerings[iPos].Sponsor);
        ...
        public StringBuilder GenerateFooter(PrintBaseModel printBaseModel)
        {
            ...
            StringBuilder footerHTML = new StringBuilder("<div class='col-md-12'><table style='width:100%'><tr><td colspan='3'><hr /></td></tr><tr><td style='text-align: left'>Offering report</td><td style='text-align: center'>&nbsp;" +
